...
class Segmenter(MessagePassing):
    ...
    def forward(
        ...
        n_nodes: int,
        max_prop_iter: typing.Optional[int] = None,
        ...
        x = torch.arange(n_nodes, dtype=torch.long, device=edge_index.device).view(
            -1, 1
        )
        for _ in range(n_iter):
            x = self.propagate(edge_index, x=x, edge_pred=edge_pred)

        y = torch.where(x[edge_index[0]] == x[edge_index[1]], 0.0, 1.0)

        return x.view(-1), y.view(-1)

    def message(
        self, x_j: torch.Tensor, x_i: torch.Tensor, edge_pred: torch.Tensor
    ) -> torch.Tensor:
        return torch.where(edge_pred < self.threshhold, x_j[:, 0], x_i[:, 0]).view(
            -1, 1
        )
